...
if __name__ == "__main__":
    ...
    data_train = datasets.TrajectoryCoordVideosDataset(videos_folder=config.VIDEOS_ROOT_DIR, data_list=train_data_full,
    ...
    data_val = datasets.TrajectoryCoordVideosDataset(videos_folder=config.VIDEOS_ROOT_DIR, data_list=val_data_full,
                                          seq_length=seq_length,
    ...
    # Load Model
    model_name = "CNNTrBimDM" + str(config.DIM_RESIZE[0]) + "skf_" + str(config.SKIPPED_FRAMES) + "d_" + str(
        config.DURATION)
    ...
    nw = models.CNN_TransformerBimodal(backbone_name=config.BACKBONE, seq_length=seq_length,
                                       n_classes=config.N_CLASSES, dim=config.DIM, depth=config.DEPTH,
                                       heads=config.HEADS,mlp_dim=config.MLP_DIM, dim_head=config.DIM_HEAD,
...
#     print(nw)
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        for batch in tqdm(dataloader_train):
            ...
            coords_batch = batch[1].to(device)
            labels = batch[2].to(device)
            pre = nw(imgs_batch, coords_batch)
